...
function bukaModalQty(p) {
    ...
    if (p.name.toLowerCase().includes('ote')) {
        ...
        setTimeout(() => { document.getElementById('input-qty-biji').focus(); document.getElementById('input-qty-biji').select(); }, 50);
    } else {
        const ada = cart.find(i => i.id === p.id);
        if (ada) { ada.qty += 1; } else { cart.push({ ...p, qty: 1, masukWaitingList: p.type === 'dadakan' }); }
        renderCart();
    }
}
...
function bukaModalCash() {
    ...
    document.getElementById('input-uang-pas').value = ''; document.getElementById('teks-kembalian').innerText = 'Kembali: Rp 0';
    document.getElementById('btn-konfirm-cash').disabled = true; document.getElementById('modalCash').style.display = 'flex';
    setTimeout(() => { document.getElementById('input-uang-pas').focus(); }, 50);
}
function hitungKembalian() {
    ...
    if (uang >= totalTagihanGlobal) { document.getElementById('teks-kembalian').innerText = `Kembali: Rp ${sisa.toLocaleString('id-ID')}`; document.getElementById('teks-kembalian').style.color = '#28a745'; btn.disabled = false; } 
